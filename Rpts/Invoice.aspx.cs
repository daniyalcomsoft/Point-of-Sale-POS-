...
public partial class Rpts_Invoice : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            ...
            if (dtRole.Rows.Count > 0)
            {
                ...
                {
                    if (!String.IsNullOrEmpty(Request.QueryString["SaleNo"]))
                    {
                        int SaleNo = Convert.ToInt32(Request.QueryString["SaleNo"]);
                    ConfigureReports(SaleNo);
                    }
                }
                else
                {
    ...
    private void ConfigureReports(int SaleNo)
    {
        try
        {
            ...
            dt = BL.GetSalesInvoice(SaleNo);
            ReportViewer1.ProcessingMode = ProcessingMode.Local;    
            
            if(dt.Rows.Count>0)
            {
                string saletype = dt.Rows[0]["SaleType"].ToString();
                if(saletype== "TakeAway")
                    ReportViewer1.LocalReport.ReportPath = Server.MapPath("~//Rpts//rdlcReports//SaleInvoice.rdlc");
                else if (saletype == "Delivery")
                    ReportViewer1.LocalReport.ReportPath = Server.MapPath("~//Rpts//rdlcReports//DeliverySaleInvoice.rdlc");
                else if (saletype == "Dining")
                    ReportViewer1.LocalReport.ReportPath = Server.MapPath("~//Rpts//rdlcReports//DiningSaleInvoice.rdlc");
